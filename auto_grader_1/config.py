...
# Helper function: Convert string to boolean (True/False)
...
# Helper function: Convert string to integer (return None if empty)
...
# Helper function: Convert string to float
...
# Load .env file using python-dotenv library (preferred method)
...
# Fallback: Read .env file line by line manually if the library is not available
...
# Find the first file in a folder (used as fallback)
def _first_file(path: Path) -> Path:
    files = sorted(p for p in path.glob("*") if p.is_file())
    return files[0] if files else Path("")


# Find the answer file that contains the assignment ID in its filename
...
# Try to load .env using the library first, fallback to manual method
...
@dataclass
class Settings:
    """Central configuration class for the entire Auto Grader project."""
    ...
    # Vision (image/PDF) processing settings
    ...
    # Grading rules (AI will follow this when scoring)
    ...
    def answer_file(self) -> Path:
        """Automatically find the standard answer file."""
    ...
    def resolved_llm_api_url(self) -> str:
        """Automatically build the full LLM chat/completions URL."""
        if self.llm_api_url:
